	...
	*/
	...
	render() {
		...
		const clientData = map(
			authorizedJobStore.clientNamesWithoutDupes,
			(item, i) => ({ key: i, label: item })
		);
		...
		const subTaskData = map(
			authorizedJobStore.subTasksWithoutDupes,
			(item, i) => ({ key: i, label: item })
		);
		subTaskData.unshift({ key: -3, section: true, label: 'Sub-Tasks' });
		// Add data to jobNumberData from authorizedJobStore
		const jobNumberData = map(
			authorizedJobStore.jobNumberWithoutDupes,
			(item, i) => ({ key: i, label: item })
		);
			...
			<Container>
				...
				<Content>
					...
					{todaysJobStore.hasUncommitted &&
						...
						>
							WARNING! You have uncommitted changes. Please go to the
							'Today's Charges' tab and click the 'Update Charges'
							button. If you proceed with Adding a new Entry, your
					...
					{(authorizedJobStore.jobNumberSize === 1) &&
							...
							>
								...
								<View>
									<Input
										...
										maxLength={6}
										onSubmitEditing={() =>
											authorizedJobStore.addEntry(navigate)}
									/>
								</View>
							</Grid>}
						...
						{/*Add Charge Button*/}
						{authorizedJobStore.hours !== null &&
							authorizedJobStore.hours !== '' &&
							!isNaN(authorizedJobStore.hours) &&
							<Button
								block
						...
						{(authorizedJobStore.hours === null ||
							...
							<View style={{ paddingHorizontal: 35, marginBottom: 60 }}>
								<Grid style={{ justifyContent: 'center' }}>
									...
									>
										Click the bottom-most field and make a selection
